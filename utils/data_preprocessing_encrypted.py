...
final_csv_path = '../data/samples/preprocessed_data_encrypted.csv'
        ...
        for packet in packets:
            ...
            if IP in packet:
                if UDP in packet:
                    ...
                    payload_length = udp_total_length - udp_header_length
...
cur = conn.cursor()
cur.execute('SELECT url, metadata FROM row_data WHERE metadata LIKE "%vpn%"')
rows = cur.fetchall()
